...
if __name__ == "__main__":
    ...
    plot_function(input_signal, output_signal, 1, name='step')

    # Tanh
    # f(a) = tanh(a) = 2 / (1+e^(-2a)) - 1
    output_signal = [2 / (1 + np.exp(-2 * a)) - 1 for a in input_signal]
    plot_function(input_signal, output_signal, 2, name='tanh')

    # SIGMOID
    # sigmoid(a) = 1 / (1 + e^-a)
    output_signal = [1 / (1 + np.exp(-a)) for a in input_signal]
    plot_function(input_signal, output_signal, 3, name='sigmoid')

    # RELU = Rectified Linear Unit
    # f(a) = max (0, a)
    output_signal = [max(0, a) for a in input_signal]
    plot_function(input_signal, output_signal, 4, name='relu')
